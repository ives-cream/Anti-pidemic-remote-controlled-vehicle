//Motor：https://randomnerdtutorials.com/esp8266-web-server/
//right motor
...
// #include <Arduino.h>
// #include <HCSR05.h>
SoftwareSerial Arduino_SoftSerial(11, 12); //Arduino D1(RX)->12(TX),D2(TX)->11(RX)，RX接收, TX傳送
String Receive_Information, IndataOf, Information; //Information 解析過後的資料
// 定義接腳；
//左輪
...
void loop(){
    if (Arduino_SoftSerial.available()){
      // Serial.write(Arduino_SoftSerial.read());
      data = Arduino_SoftSerial.read();
      // Serial.write(data);
      }      
    distance = sr05.ping(); //超音波偵測到的距離
    // distance = 20;
    //如果偵測到前方模公分以內有障礙物時先停止0.3秒後再往後倒退1秒
    if(distance <= thresh) {// && (data == 'a' || data == 'c' || data == 'd')；如果偵測距離小於10(可以)，並且esp狀態是前左右(後者不行會產生延遲)
      Serial.println(distance);
      motor.drive(Stop, 0, 0); //停止
      delay(100);
      motor.drive(Backward, pwm, 0); //後退
      delay(1000);
    }else{
      switch (data) {
        case 'a':
          ...
          break;
        case 'b':
          motor.drive(Backward, pwm, 0);
          Serial.println("Backward");
          break;
        case 'c':
          motor.drive(Left, pwm, 0);
          Serial.println("Left");
          break;
        case 'd':
          motor.drive(Right, pwm, 0);
          Serial.println("Right");
          break;
        case 'e':
          motor.drive(Stop, 0, 0);
          Serial.println("Stop");
          break;
        default:
          break;    
