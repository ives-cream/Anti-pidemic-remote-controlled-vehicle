...
SoftwareSerial NodeMcu_SoftSerial(D1, D2); //ESP8266 D1(RX)->12(TX),D2(TX)->11(RX)，RX接收, TX傳送，rxPin, txPin
ESP8266WebServer server(80);      //在埠口80建立網站伺服器，server是個自動的伺服器物件

//wifi名稱 與 wifi密碼
String ssid = "wifi名稱";       //ivescream
String password = "密碼";       //ivescream
...
<body>\
    ...
    <table>\
        ...
        <tr>\
            <form>\
                <td> <button id=\"Left\">左轉</button> </td>\
            </form>\
            <form>\
                <td> <button id=\"Stop\">急剎</button> </td>\
            </form>\
            <form>\
        ...
        <tr>\
            <td></td>\
            <form>\
                <td><button id=\"Backward\">後退</button></td>\
            </form>\
            <td></td>\
        </tr>\
    </table>\
